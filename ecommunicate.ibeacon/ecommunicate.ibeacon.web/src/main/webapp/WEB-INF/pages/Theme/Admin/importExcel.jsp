...
<div class="container" style="position: relative;">
	...
	<div id="main">
		...
		<form id="fileupload"
			action="${pageContext.request.contextPath}/admin/uploadAreaCode"
							...
							<table
								...
								<tbody>
									<tr class="gradeA"
										...
										<td>{{$index+1}}</td>
										<td>
											<div ng-hide="editingData[area.id]">{{area.areaName |
												uppercase}}</div>
											<div ng-show="editingData[area.id]">
												<input type="text" ng-model="area.areaName" />
											</div>
										</td>
										<td>
											<div ng-hide="editingData[area.id]">{{area.blockNumber}}</div>
											<div ng-show="editingData[area.id]">
												<input type="text" ng-model="area.blockNumber" />
											</div>
										</td>
<td>
                                     <div ng-hide="editingData[area.id]">{{area.province}}</div>
											<div ng-show="editingData[area.id]">
...
<script
	src="${pageContext.request.contextPath}/assets/js/CustomJs/importExcel.js"></script>
<script
