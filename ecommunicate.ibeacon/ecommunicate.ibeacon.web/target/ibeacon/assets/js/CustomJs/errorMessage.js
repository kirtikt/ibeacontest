...
helloApp.controller("HttpController", [ '$scope', '$http',
    ...
    $scope.remove = function(area){
    	var response = $http.post(contextPath+'/admin/removeerror', area);
...
//			alert( "Exception details: " + JSON.stringify({data: data}));
    ...
    $scope.update = function(area){
    	...
    	var response = $http.post(contextPath+'/admin/updateError', area);
		response.success(function(data, status, headers, config) {
//			alert("success")
});
		response.error(function(data, status, headers, config) {
...
$scope.addNewChoice = function() {
		...
		var newItemNo = $scope.choices.length + 2;
		/*   alert( $scope.choices); */
		$scope.choices.push({
			'id' :  newItemNo
		});
	...
	debugger;
		$http({
			...
			url : contextPath+'/admin/viewAllErrorMessage',
...
//			var userId = $('#userId').val();
			...
			var response = $http.post(contextPath+'/admin/SaveNewErrorMessage', $scope.choices);
			response.success(function(data, status, headers, config) {
				if(data=='true'){
				location
				.reload(true);
